...
(require/typed/provide web-server/http/id-cookie
                       [make-id-cookie (-> Cookie-Name Bytes Cookie-Value [#:path (Option String)] Cookie)]
                       [request-id-cookie (-> Cookie-Name Bytes Request #:timeout Number (Option Cookie-Value))]
                       [logout-id-cookie (-> Cookie-Name [#:path (Option String)] Cookie)]
                       [make-secret-salt/file (-> Path-String Bytes)])

(require/typed/provide web-server/http/cookie-parse
                       [#:struct client-cookie {[name : String]
                                                [value : String]
                                                [domain : (Option Valid-Domain)]
                                                [path : (Option String)]}]
...
(require/typed/provide web-server/http/basic-auth
                       ...
                       [request->basic-credentials (-> Request (Option (Pairof Bytes Bytes)))])
